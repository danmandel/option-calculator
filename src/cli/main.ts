...
import { AlpacaMarketDataProvider } from "../services/marketData";
import { formatUSD } from "./render/format";
import { buildPayoffTable } from "./render/table";
...
export const main = async (): Promise<void> => {
  ...
  const market = new AlpacaMarketDataProvider();
  ...
  try {
    ...
    const spreadQuote = await market.getSpreadMidDebit({
      symbol,
      longStrike,
      shortStrike,
      expiration: expiry,
    ...
    if (spotPrice === undefined) {
      try {
        spotPrice = await market.getSpot(symbol);
      } catch {
    ...
    if (tableCsv || tableRange) {
      ...
      const table = buildPayoffTable({
        prices,
        longStrike,
        shortStrike,
        debit,
        contractSize,
        spreadContracts: sizedContracts,
        portfolio,
      ...
      console.log("Spread portfolio profit at cap:", formatUSD(totalMaxProfit));
      if (includeStockBenchmark && stockProfitAtHighest !== undefined) {
